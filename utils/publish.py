...
class Publish:
        ...
        if optimize.flag_excluded:
            ...
            for engine, idx in enumerate(optimize.cons_idx):
                if idx is not None:
                    ...
                    self.mqttc.publish(f"mpei/DGU/{engine + 1}/Consuming", optimize.input_L_J[idx, engine])
                    self.mqttc.publish(f"mpei/DGU/{engine + 1}/Power_DGU/current_generator_power",
            ...
            for engine in range(6):
                if engine not in list_engine:
                    self.mqttc.publish(f"mpei/DGU/{engine + 1}/Power_DGU/current_generator_power", 0)
    ...
    def regulation_frequency(self, frequency):
        if frequency.flag_frequency:
            ...
            self.mqttc.publish(f"mpei/DGU/Power/Power_gain", frequency.Delta_P)
            self.mqttc.publish(f"mpei/DGU/Power/Your_power", frequency.P_DES_new)
            self.mqttc.publish(f"mpei/DGU/Power/Current_power", frequency.load)
